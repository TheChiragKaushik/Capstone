...
@Service
@EnableScheduling
public class PatientNotificationsService {
	...
	public void sendScheduledNotification(PatientNotificationsRequestsEO request) {
		try {
			...
			System.out.println("Scheduled Notification: Attempted to send to " + request.getPatientId()
					+ " via STOMP to /user/" + request.getPatientId() + "/notifications");

			updateNotificationRequestStatus(request.get_id_asString(), "SENT")
					.then(savePatientNotification(request))
